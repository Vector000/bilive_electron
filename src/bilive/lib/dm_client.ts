...
class DMclient extends EventEmitter {
  ...
  protected _ClientConnectHandler() {
    ...
    if (this._protocol === 'socket')
      data = JSON.stringify({ roomid: this.roomID, uid: this.userID, from: 0, platform: 'android', clientver: '5.31.3.5310300', hwid: AppClient.DeviceID, protover: 2 })
  ...
  protected async _ParseClientData(data: Buffer) {
    switch (data.readInt32BE(8)) {
      case 3:
        ...
        break
      case 5: {
        const dataJson = await JSON.parse(data.toString('UTF-8', 16))
   ...
   * @memberof DMclient
   */
  protected _ClientData(dataJson: any) {

...
function Options(options?: _options): Promise<_options> {
  return new Promise(async resolve => {
    ...
    const hasDir = await FSexists(dirname + '/options/') // 检查是否有options目录
    if (!hasDir) await FSmkdir(dirname + '/options/')
    if (options === undefined) {
      // 复制默认设置文件到用户设置文件
      const hasFile = await FSexists(dirname + '/options/options.json')
      if (!hasFile) await FScopyFile(dirname + '/src/bilive/options.default.json', dirname + '/options/options.json')
      // 读取默认设置文件
      const defaultOptionBuffer = await FSreadFile(dirname + '/src/bilive/options.default.json')
      ...
      const blacklist = ['newUserData', 'info', 'giftID', 'apiIPs', 'roomList']
...
function Log(...message: any[]) {
  ...
  allContents.forEach((windowContent) => {
    windowContent.send('MTOR', { cmd: 'log', msg: message })
  })
}
...
export default { testIP, XHR, setCookie, getCookie, Options, getShortRoomID, getLongRoomID, JSONparse, Hash, Log, ErrorLog, sendSCMSG, Sleep }
export { response }
